...
export class Board extends React.Component {
    ...
    moveY(direction) {
        if (direction === '+'
            ...
            Log.info('Moving in +y directions');
        ...
        } else if (direction === '-' && (this.state.marioLocation + this.props.height) < this.totalSquares) {
            Log.info('Moving in -y direction');
            let newSquares = this.state.squares.slice();
            newSquares[this.state.marioLocation] = {
                element: '', display: 'none'
            };
    ...
    moveX(direction) {
        if (direction === '+' && (this.state.marioLocation + 2) % this.props.width !== 1
            && (this.state.marioLocation + 1) < this.totalSquares) {
            Log.info('Moving in +x directions');
            let newSquares = this.state.squares.slice();
            newSquares[this.state.marioLocation] = {
                element: '', display: 'none'
            };
            ...
            if (direction === '-' && (this.state.marioLocation) % this.props.width !== 0
                && (this.state.marioLocation - 1) >= 0) {
                Log.info('Moving in -x direction');
                let newSquares = this.state.squares.slice();
                newSquares[this.state.marioLocation] = {
                    element: '', display: 'none'
                };
    ...
    sortEnemies (enemies) {
        enemies.map((enemy) => {
            let factor = Math.round(Math.abs((enemy.value - this.state.marioLocation)/10));
            let num = 0;
            if (this.state.marioLocation > enemy.value)
                num = factor + Math.abs(this.state.marioLocation - (factor*this.props.height)-enemy.value);
            else
                num = factor + Math.abs(this.state.marioLocation + (factor*this.props.height)-enemy.value);
            
                enemy.marioDistance = num;
        });
        
        enemies = enemies.sort((a, b) => a.marioDistance < b.marioDistance?-1:1);
        enemies.map((enemy) => Log.info(`Enemies sort array: ${enemy.value}`));
        return enemies;
    }

    checkForEnemies() {
        Log.info('checking for enemies: ', this.state);
        let enemies = this.state.squares.filter(square => {
            return square.element === 'enemy.png';
        });

        enemies = this.sortEnemies(enemies);

        Log.info('Total enemies: ', enemies);
        if (enemies.length === 0)
            alert('You win. Total moves to save princess: ' + this.state.moves);
        else
    ...
    renderSquare(i, element, display) {
        return <Square key={i}
                    value={i} element={this.state.squares[i].element} 
    ...
    render() {
        ...
        Log.info('calculating luck squares: ', luckySquares);
        let squareNumber = 0;
        for (let i = 0; i < this.props.height; i++) {
            for (let j = 0; j < this.props.width; j++) {
